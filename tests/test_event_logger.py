...
class TestSQLiteEventLogger:
    ...
    async def test_get_events_with_limit(self, temp_logs_dir):
        ...
        for i in range(10):
            await logger.log_event(f"city{i}", datetime.utcnow(), f"/path{i}.json", False)
...
class TestEventLogger:
    ...
    async def test_get_request_history_with_filters(self, event_logger):
        ...
        await event_logger.log_weather_request("berlin", datetime.utcnow(), "/a.json", False)
        await event_logger.log_weather_request("rome", datetime.utcnow(), "/b.json", False)
